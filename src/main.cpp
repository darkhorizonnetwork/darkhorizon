...
#if defined(NDEBUG)
# error "DarkHorizon cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6ca0502976ecf284f7bb15051252fa6aa80ad40cfe400ac4df8ca11e460d0eee");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // DarkHorizon: starting difficulty is 1 / 2^12
...
int64 CTransaction::nMinTxFee = 1000; //100000;
/** Fees smaller than this (in satoshi) are considered zero fee (for relaying) */
int64 CTransaction::nMinRelayTxFee = 1000; //100000;
...
const string strMessageMagic = "DarkHorizon Signed Message:\n";
...
{
    // DarkHorizon: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
  // coinbase maturity is now 5 blocks -- removed ""+20"
    if (!IsCoinBase())
        return 0;
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if( nHeight == 1 ){
      nSubsidy = 60 * COIN;
    } else if( nHeight > 1 && nHeight <= 602 ){
      nSubsidy = 0.1 * COIN;
    } else if( nHeight > 602 && nHeight <= 54603 ){
      nSubsidy = 0.01 * COIN;
    } else if( nHeight > 54603 && nHeight <= 59394604 ){
      nSubsidy = 0.001 * COIN;
    }
...
static const int64 nTargetTimespan = 10; // 10 second block calculation //7 * 24 * 60 * 60; // 5 min //3.5 * 24 * 60 * 60; // DarkHorizon: 3.5 days
static const int64 nTargetSpacing = 10; // 10 second spacing //7 * 24 * 60 * 60; // DarkHorizon: 7 days
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // DarkHorizon: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // DarkHorizon: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x10258265b6afa700ded0a15a2ca4fbb962e4b8bb2a125866231bdc9fbb6deab9");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "KR Times 25/Feb/2018 Hanwha, KAIST to develop AI weapons";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04434f10b15a0b8097acae9cb33b6d32502236b01fcd48b6352fd20d1925938de5c6a5bcef9a3bb49a2240543fcdc51be10f5b5fae01d2f5081a7f196c6b950e8f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1519793437;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087598688;

        if (fTestNet)
        {
            block.nTime    = 1519793426;
            block.nNonce   = 387579956;
        }
...
/// mine gen block end
        ...
        assert(block.hashMerkleRoot == uint256("0xabcc57950eaeebfce10e63a6dd5be938253c6367b43361e841dd0a310b733ece"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf5, 0xb5, 0xc0, 0xd5 }; // DarkHorizon.
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!vecPriority.empty())
        {
            ...
            // transactions:
            if (!fSortedByFee &&
                ((nBlockSize + nTxSize >= nBlockPrioritySize) || (dPriority < COIN * 1 / 1000))) //was 576 /250
            {
                fSortedByFee = true;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DarkHorizonMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DarkHorizonMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DarkHorizonMiner(CWallet *pwallet)
{
    printf("DarkHorizonMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("darkhorizon-miner");
    ...
    try { loop {
        ...
        printf("Running DarkHorizonMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DarkHorizonMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DarkHorizonMiner, pwallet));
}
